...
const MotionNavigationMenuContext = React.createContext<MotionNavigationMenuContextValue | null>(null);
const MotionNavigationMenuItemContext = React.createContext<MotionNavigationMenuItemContextValue | null>(null);
...
type MotionNavigationMenuProps = Omit<React.ComponentPropsWithRef<'div'>, 'onValueChange'> & {
  viewport?: boolean;
...
// Adapted from a pasted "motion navigation menu" reference component:
// - framer-motion -> motion/react (this project already depends on `motion`, not framer-motion)
// - root element nav -> div (this sits inside Navbar's own top-level <nav>; nesting <nav> landmarks is discouraged)
// - the "@/components/unlumen-ui/primitives/highlight" dependency (not provided) is replaced with the local, simplified ./highlight stand-in
function MotionNavigationMenu({
  ...
  const [contentByValue, setContentByValue] = React.useState<Record<string, ContentRecord>>({});
  ...
  const getItemValues = React.useCallback(() => {
    ...
    if (!root) return [];
    return Array.from(
      root.querySelectorAll<HTMLElement>('[data-slot="navigation-menu-item"][data-value]'),
  ...
  const updateViewportPosition = React.useCallback(() => {
    ...
    frameRef.current = requestAnimationFrame(() => {
      ...
      const activeTrigger = root.querySelector<HTMLElement>('[data-slot="navigation-menu-trigger"][data-state="open"]');
      ...
      const contentWidth = (measureEl ? measureEl.offsetWidth : 0) || (viewportEl ? viewportEl.offsetWidth : 0);
      ...
      if (contentWidth > 0) {
        ...
        if (!boundary) boundary = document.documentElement.getBoundingClientRect();
  ...
  const registerContent = React.useCallback((value: string, content: ContentRecord) => {
  ...
  const contextValue = React.useMemo(
    ...
    [activeValue, closeMenu, direction, openValue, registerContent, spring, updateViewportPosition, viewport, viewportX],
  );
  ...
  return (
    <MotionNavigationMenuContext.Provider value={contextValue}>
      ...
      >
        {children}
        {viewport && <MotionNavigationMenuViewport className={viewportClassName} contentByValue={contentByValue} />}
      </div>
...
function MotionNavigationMenuList({
  ...
  highlightClassName,
  ...props
}: React.ComponentPropsWithRef<'ul'> & { highlightClassName?: string }) {
  return (
    <Highlight
      mode="parent"
      controlledItems
      hover
      className={cn('bg-white/10 rounded-md pointer-events-none', highlightClassName)}
      style={{ zIndex: -1 }}
      containerClassName="relative"
    >
      <ul
        data-slot="navigation-menu-list"
        className={cn('group relative z-10 flex flex-1 list-none items-center justify-center gap-1', className)}
        {...props}
      />
    </Highlight>
  );
...
function MotionNavigationMenuItem({
  ...
  value,
  ...props
}: React.ComponentPropsWithRef<'li'> & { value?: string }) {
...
const motionNavigationMenuTriggerStyle = cva(
  'group inline-flex h-9 w-max items-center justify-center rounded-md bg-transparent px-4 py-2 text-sm font-medium hover:text-gold focus:text-gold disabled:pointer-events-none disabled:opacity-50 data-[state=open]:text-gold outline-none transition-colors focus-visible:ring-2 focus-visible:ring-gold/50',
);
...
function MotionNavigationMenuContent({
  ...
  innerClassName,
  children,
}: React.ComponentPropsWithRef<'div'> & { highlightClassName?: string; innerClassName?: string }) {
  const context = React.useContext(MotionNavigationMenuContext);
  const itemContext = React.useContext(MotionNavigationMenuItemContext);
  const value = itemContext?.value;
  const isOpen = !!value && context?.activeValue === value;
  ...
  React.useLayoutEffect(() => {
    ...
    return context.registerContent(value, { children, className, highlightClassName, innerClassName });
    ...
    <AnimatePresence initial={false} custom={context.direction}>
      {isOpen && (
        <motion.div
          ...
          className={cn('bg-cinematic-black/95 text-white absolute top-full left-0 z-50 mt-1.5 rounded-md border border-white/10 p-2 pr-2.5 shadow', className)}
        >
          <MotionNavigationMenuContentInner highlightClassName={highlightClassName} innerClassName={innerClassName}>
            {children}
...
function MotionNavigationMenuContentInner({
  ...
  return (
    <Highlight
      ...
      controlledItems
      hover
      className={cn('bg-white/10 rounded-sm pointer-events-none', highlightClassName)}
      style={{ zIndex: -1 }}
      containerClassName="relative"
    >
...
function MotionNavigationMenuViewport({
  className,
  contentByValue,
}: React.ComponentPropsWithRef<'div'> & { contentByValue?: Record<string, ContentRecord> }) {
  const context = React.useContext(MotionNavigationMenuContext);
  const measureRef = React.useRef<HTMLDivElement | null>(null);
  const [size, setSize] = React.useState({ width: 0, height: 0 });
  const [lastSize, setLastSize] = React.useState({ width: 0, height: 0 });
  const activeContent = context?.activeValue && contentByValue ? contentByValue[context.activeValue] : undefined;
  ...
  return (
    ...
    >
      <motion.div
        ...
        transition={context?.spring}
        className={cn('bg-cinematic-black/95 text-white relative mt-1.5 overflow-hidden rounded-md border border-white/10 shadow backdrop-blur-md', className)}
      >
        <AnimatePresence mode="popLayout" initial={false} custom={context?.direction ?? 1}>
          {activeContent && context?.activeValue && (
            ...
            >
              <MotionNavigationMenuContentInner highlightClassName={activeContent.highlightClassName} innerClassName={activeContent.innerClassName}>
      ...
      <div ref={measureRef} aria-hidden="true" data-slot="navigation-menu-measure" className="pointer-events-none invisible absolute top-1.5 left-0 w-max">
        {activeContent && (
