    ...
    return (
      <Layout>
        <div className="min-h-screen flex items-center justify-center bg-transparent">
          <div className="w-16 h-16 border-2 border-[#5490B4]/10 border-t-[#5490B4] rounded-full animate-spin" />
        </div>
      </Layout>
    );
  }
    ...
    return (
      <Layout>
        <div className="min-h-screen flex flex-col items-center justify-center bg-transparent px-[5%] text-center">
          ...
          <button onClick={() => navigate('/artists')} className="px-16 py-6 bg-[#5490B4] text-[#1A1A1A] text-[10px] font-black uppercase tracking-[0.5em] hover:bg-[#5490B4]/90 transition-all duration-700 font-sans">
            RETURN TO REGISTRY
          </button>
        </div>
      </Layout>
    ...
    <Layout>
      <section className="pt-48 pb-40 bg-transparent min-h-screen relative overflow-hidden">
        ...
        <div className="layout-container relative z-10 text-[#F5F5F5]">
          <button 
            onClick={() => navigate(-1)}
            className="mb-16 flex items-center gap-4 text-white/40 hover:text-[#5490B4] transition-colors group"
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-20 lg:gap-32 items-start">
            ...
            <div className="lg:col-span-4 space-y-12">
              ...
              >
                ...
                <p className="text-[10px] text-[#5490B4] font-bold uppercase tracking-[0.5em] mb-12">VERIFIED CREATOR</p>
                  ...
                  {profile.links && profile.links.length > 0 ? (
                    profile.links.map((link, i) => (
                      <a 
                        ...
                        className="flex-1 min-w-[120px] bg-white/5 border border-white/10 rounded-xl px-4 py-4 flex items-center justify-between group/link hover:border-[#5490B4]/50 transition-all duration-500"
                      >
                        <span className="text-[10px] text-white/60 uppercase font-bold tracking-widest truncate group-hover/link:text-[#5490B4]">{link.label || 'Link'}</span>
                        <ExternalLink size={12} className="text-white/20 group-hover/link:text-[#5490B4]" />
                      </a>
                    ))
                  ) : (
                ...
                <div className="space-y-4">
                  <button 
                    ...
                    className="w-full py-6 bg-[#5490B4] text-[#1A1A1A] text-[10px] font-black uppercase tracking-[0.4em] flex items-center justify-center gap-4 hover:bg-[#5490B4]/90 transition-all duration-500 shadow-xl shadow-black/10 font-sans"
                  >
                    INITIATE CHANNEL <MessageSquare size={16} />
                  </button>
                  <button className="w-full py-6 border border-white/10 text-[10px] font-bold uppercase tracking-[0.4em] text-white/50 hover:text-[#5490B4] hover:border-[#5490B4]/55 rounded-none transition-all font-sans">
              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-12 text-center backdrop-blur-md">
                ...
                <p className="text-4xl font-black text-white tracking-tighter">
                  {profile.coins || 0} <span className="text-[#5490B4] text-xs uppercase font-black tracking-widest ml-2">SOSRG</span>
                </p>
              </div>
            </div>
            ...
            <div className="lg:col-span-8">
              ...
              >
                <div className="space-y-10">
                  <div className="flex items-center gap-6 text-[#5490B4]">
                    <Sparkles size={20} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16">
                  <div className="space-y-8">
                    ...
                    <div className="flex flex-wrap gap-4">
                      {profile.skills?.map(skill => (
                        <span key={skill} className="px-8 py-3 bg-white/5 border border-white/10 rounded-lg text-[#5490B4] text-[10px] font-bold uppercase tracking-[0.2em] italic hover:border-[#5490B4] transition-all shadow-sm">
                          {skill}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="flex items-center gap-6 p-8 bg-white/5 border border-white/10 rounded-xl">
                      <MapPin size={18} className="text-[#5490B4]" />
                ...
                <div className="mt-40 pt-40 border-t border-white/10">
                  <div className="flex items-center justify-between mb-16">
                    <h2 className="text-3xl md:text-5xl font-bold text-white tracking-tighter premium-serif">Project <span className="text-[#5490B4] italic font-light">Archives.</span></h2>
                    <span className="text-[10px] text-white/30 uppercase font-bold tracking-[0.4em]">SYNCHRONIZING...</span>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    {[1, 2].map(i => (
                      <div key={i} className="aspect-video bg-white/5 border border-white/10 rounded-2xl flex flex-col items-center justify-center group cursor-pointer overflow-hidden shadow-sm hover:shadow-2xl hover:border-[#5490B4]/50 transition-all duration-700 backdrop-blur-md">
